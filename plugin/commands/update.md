...
1. `ds version` — record the version installed right now, so there is something to compare against.
...
3. `ds version` again and compare. If the number did not move, say so plainly rather than implying
   an update happened: either they were already on the latest, or the update did not take.

4. If the version did move, run `ds sync --strict`. A release can change what the model format
