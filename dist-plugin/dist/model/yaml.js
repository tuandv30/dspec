...
function scan(src, where, firstLine = 1) {
    const out = [];
    const raw = src.split('\n');
    for (let i = 0; i < raw.length; i++) {
        ...
        const lead = /^[ \t]*/.exec(l)[0];
        if (lead.includes('\t'))
            throw new YamlError('tab indentation — use spaces', firstLine + i, where);
...
function splitFlow(inner, no, where) {
    const parts = [];
    let cur = '';
    let quote = null;
    ...
    if (quote)
        throw new YamlError('unclosed quote', no, where);
    if (cur.trim() || parts.length)
        parts.push(cur);
    return parts.map((p) => p.trim()).filter((p) => p !== '');
}
function parseScalar(s, no, where) {
    const t = s.trim();
    if (t === '' || t === '~' || t === 'null')
        return null;
    if (t === 'true')
    ...
    if (t.startsWith('"')) {
        if (!t.endsWith('"') || t.length < 2)
            throw new YamlError('unclosed double quote', no, where);
        return t.slice(1, -1).replace(/\\(["\\ntr])/g, (_, c) => c === 'n' ? '\n' : c === 't' ? '\t' : c === 'r' ? '\r' : c);
    }
    if (t.startsWith("'")) {
        if (!t.endsWith("'") || t.length < 2)
            throw new YamlError('unclosed single quote', no, where);
        return t.slice(1, -1).replace(/''/g, "'");
    }
    if (t.startsWith('[')) {
        if (!t.endsWith(']'))
            throw new YamlError('flow sequence missing its `]`', no, where);
        return splitFlow(t.slice(1, -1), no, where).map((p) => parseScalar(p, no, where));
    }
    if (t.startsWith('{'))
        throw new YamlError('flow map `{…}` is not supported — use a nested map', no, where);
    if (t === '|' || t === '>')
        throw new YamlError('block scalar `|`/`>` is not supported — use double quotes', no, where);
...
function parseBlock(lines, start, indent, where) {
    ...
    while (i < lines.length && lines[i].indent === indent) {
        ...
        if (!kv)
            throw new YamlError(`not a \`key: value\` pair: ${JSON.stringify(line.text)}`, line.no, where);
        if (!kv.key)
            throw new YamlError('empty key', line.no, where);
        if (kv.key in map)
            throw new YamlError(`duplicate key \`${kv.key}\``, line.no, where);
        if (kv.rest) {
    ...
    if (i === start)
        throw new YamlError(`inconsistent indentation: ${JSON.stringify(lines[i].text)}`, lines[i].no, where);
    return [map, i];
}
/**
 ...
 */
function parseYaml(src, where, firstLine = 1) {
...
function emit(v, indent, out) {
    ...
    if (Array.isArray(v)) {
        ...
        if (v.every(isScalar)) {
            ...
            if (flat.length + indent <= 72) {
                out[out.length - 1] += ` ${flat}`;
                return;
            }
        }
        for (const item of v) {
