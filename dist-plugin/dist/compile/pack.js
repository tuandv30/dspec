...
function renderContract(f) {
    ...
    out.push('');
    if (f.lead.trim())
        out.push(f.lead.trim(), '');
    if (f.rules.length)
        out.push('Rules', ...f.rules, '');
...
function renderPack(repo, model, request, opts = {}) {
    ...
    if (used.length) {
        lines.push('## What it uses', '', '_One hop out. These are declared dependencies, not guesses — changing one changes this._', '_Each is a **contract**: what it promises and the rules it holds you to, not how it works._', '');
        for (const f of used)
            lines.push(...renderContract(f));
    }
