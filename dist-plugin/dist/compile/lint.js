...
exports.SEVERITY = exports.LINT_RULE_CODES = void 0;
exports.lintModel = lintModel;
exports.lintRepo = lintRepo;
exports.featuresClaiming = featuresClaiming;
...
const finding = (code, detail, extra = {}) => ({
    code, severity: exports.SEVERITY[code], detail, ...extra,
});
/**
...
function lintModel(model) {
    const out = [];
    const names = new Set(model.features.map((f) => (0, types_1.normName)(f.name)));
    ...
    for (const f of model.features) {
        const key = (0, types_1.normName)(f.name);
        const g = groups.get(key);
        if (g)
            g.count++;
        else
            groups.set(key, { label: f.name, count: 1 });
    }
    ...
    for (const f of model.features) {
        ...
        for (const u of f.uses) {
            if (!names.has((0, types_1.normName)(u))) {
...
function featuresClaiming(model, file) {
    return (0, types_1.claims)(model.features).get(file) ?? [];
}
