...
Run `node "${CLAUDE_PLUGIN_ROOT}/bin/ds.js" sync --write`. It restores base files that have gone missing, re-stamps every feature and
...
Finish with `node "${CLAUDE_PLUGIN_ROOT}/bin/ds.js" sync --strict`: it exits non-zero only on a measured fact, so a clean run means
